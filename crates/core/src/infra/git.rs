...
impl GitOps for RealGit {
    ...
    async fn delete_branch(&self, repo: &Path, branch: &str) -> Result<()> {
        run_git(repo, &delete_branch_args(branch))
            ...
            .map(|_| ())
            .map_err(|e| {
                // Git's refusal changes wording across versions ("used by
                // worktree at" vs "checked out at"); name the worktree conflict
                // ourselves so callers don't depend on the git version.
                let msg = e.to_string();
                if msg.contains("used by worktree") || msg.contains("checked out at") {
                    CoreError::other(format!(
                        "branch '{branch}' is still checked out in a worktree: {msg}"
                    ))
                } else {
                    e
                }
